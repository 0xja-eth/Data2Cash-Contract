...
interface Vm is VmSafe {
    ...
    // memory is written to, the gmx-test will fail. Can be called multiple times to add more ranges to the set.
    function expectSafeMemory(uint64 min, uint64 max) external;
    // Only allows memory writes to offsets [0x00, 0x60) ∪ [min, max) in the next created subcontext.
    // If any other memory is written to, the gmx-test will fail. Can be called multiple times to add more ranges
    // to the set.
